...
def build_plotly_colorscale(legend, r_min, r_max):
    """
    Membuat colorscale Plotly yang valid:
    - Posisi [0,1] selalu monoton naik dan unik
    - Selalu diawali 0.0 dan diakhiri 1.0
    - Tidak crash walau legend tidak overlap dengan rentang data
    """
    if not legend:
        return [[0.0, "#1e90ff"], [1.0, "#C0C0C0"]]

    r_min = max(float(r_min), 0.01)
    r_max = max(float(r_max), r_min + 0.01)
    log_min = np.log10(r_min)
    log_max = np.log10(r_max)
    log_range = log_max - log_min if log_max != log_min else 1.0

    # Kumpulkan breakpoint dari setiap litologi
    breakpoints = []
    for item in legend:
        rmin_l = max(float(item["rmin"]), r_min)
        rmax_l = min(float(item["rmax"]), r_max)
        if rmin_l >= rmax_l:
            continue  # tidak overlap, skip
        pos_s = float(np.clip((np.log10(rmin_l) - log_min) / log_range, 0.0, 1.0))
        pos_e = float(np.clip((np.log10(rmax_l) - log_min) / log_range, 0.0, 1.0))
        breakpoints.append((pos_s, item["warna"]))
        breakpoints.append((pos_e, item["warna"]))

    # Jika tidak ada breakpoint valid, buat gradient sederhana
    if not breakpoints:
        return [[0.0, legend[0]["warna"]], [1.0, legend[-1]["warna"]]]

    # Urutkan dan buang duplikat posisi (simpan warna terakhir per posisi)
    breakpoints.sort(key=lambda x: x[0])
    seen = {}
    for pos, color in breakpoints:
        seen[round(pos, 6)] = color
    breakpoints = sorted(seen.items())  # [(pos, color), ...]

    # Pastikan selalu ada 0.0 dan 1.0
    ...
    if len(breakpoints) < 2:
        c = breakpoints[0][1] if breakpoints else "#1e90ff"
        return [[0.0, c], [1.0, c]]

    return [[float(pos), color] for pos, color in breakpoints]
